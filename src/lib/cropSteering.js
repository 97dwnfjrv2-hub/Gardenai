...
export function evaluateSteering({ drybackPct, runoffEcDelta, currentVpd, setupConfirmed }) {
  const findings = []
  if (!setupConfirmed) {
    findings.push({
      severity: 'required',
      text: 'Confirm the active pot setup before irrigation recommendations: 8 × 15 L and 4 × 30 L behave differently.'
    })
  }
  if (drybackPct == null) {
  ...
  if (runoffEcDelta > 0.5) {
    findings.push({ severity: 'watch', text: 'Runoff EC is separating materially from input EC; investigate shot size, frequency, and substrate accumulation trend.' })
  }
  if (currentVpd > 1.5) {
    findings.push({ severity: 'watch', text: 'VPD is elevated for a heavily lit room; confirm leaf temperature and transpiration response before increasing light or stress.' })
  }
  return findings
}
